...
func checkUser(email string, password string) (bool, error, int) {
	InitDB()
	var fetched_password string
	var fetched_id int
	err := DB.QueryRow("select id, password from Users where email = ?", email).Scan(&fetched_id, &fetched_password)
	if err != nil {
		if err != sql.ErrNoRows {
			return false, err, fetched_id
		}
	}
	if password == fetched_password {
		return true, nil, fetched_id
	} else {
		return false, nil, fetched_id
	}
}
